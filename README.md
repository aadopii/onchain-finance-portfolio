...
rebalanced across Base and Ethereum inside on-chain permissions only the owner controls. Built on
...
Six protocols, four verticals, two chains. Weights are global targets; the chain is wherever the
asset's liquidity lives.

| Vertical | Asset | Target | Why this one |
|---|---|---|---|
| Exchange | HYPE (held as cbHYPE) | 25% | The profitable perp exchange; ~98% of fees buy the token |
| Exchange | UNI | 17.5% | The spot venue of record; fee switch on since Dec 2025 |
| Credit | AAVE | 17.5% | The incumbent lender; revenue routed to buybacks since Jun 2026 |
| Credit | MORPHO | 17.5% | The challenger lender; an option on its fee switch |
| Yield | SKY | 17.5% | The stablecoin issuer; the closest thing onchain to net interest margin |
| Privacy | ZAMA | 5% | The confidentiality layer institutions will need; venture-sized |

Live state of the author's account on 2026-09-07, after the first full rebalance run:
...
sector trades at roughly 9x fees and 27x revenue, with revenue growing and buybacks live at four of
six. That is the whole thesis in two numbers, and it is why the position is a basket rather than a
single name: the multiple compresses for the sector, and nobody knows which vertical re-rates first.
...
Exchange gets the most because it is the vertical with proven value capture today. Credit is equal
to exchange in aggregate because it is the largest market, split evenly between the name that has
turned revenue on and the one that has not. Yield is a single name because there is one clear
leader. Privacy is a single small name because it is a bet on adoption, not on cash flow.

### What would change the thesis

Fee switches that never turn on (Morpho); a durable loss of exchange share to a new venue (HYPE);
a regulatory outcome that pushes stablecoin issuance back to banks (SKY); or a valuation reset that
...
Your assets live in a Safe (the SMA) that only you own, deployed at the same address on Base and
Ethereum. The agent holds a separate manager key that can only act through the Sail kernel, and the
...
2. **Complete mints.** For every confirmed burn, fetch Circle's attestation. If it is still pending,
   wait. If the destination transmitter already reports the burn's nonce as used, record the mint.
   Otherwise emit `receiveMessage` on the destination chain.
...
5. **Buy toward target**, in basket order. Each shortfall is bought with the idle USDC on a chain
   ...
   are pooled into one CCTP bridge per source and destination.
...
A rebalance that crosses chains is four rounds: sell, bridge, mint, buy, with Circle's attestation
(minutes to about an hour Base to Ethereum) in the middle. A single tick cannot wait. So the
...
1. **Account.** Create your Safe on Base and deploy it on Ethereum. You sign once per chain in the
...
3. **Mandate.** Compile and deploy the three permissions on each chain, simulate them against the
...
bridging to Ethereum for SKY and ZAMA.
...
| [`contracts/mandates/`](contracts/mandates/) | The three permission contracts; tests in `contracts/test/` |
